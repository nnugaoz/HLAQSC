...
{
    public class D01_PlanController : BaseController
    {
        //
        // GET: /D01_Plan/
        ...
        [HttpGet]
        public FileResult DownloadTemplate(String pFileType)
        {
            String lModelFilePath = ConfigurationManager.AppSettings["Excel_Model_Path"].ToString();
            ...
            {
                case "PLAN":
                    lTempFileName = ConfigurationManager.AppSettings["Excel_Model_Name_Plan"].ToString();
                    break;

                case "CHECK":
                    lTempFileName = ConfigurationManager.AppSettings["Excel_Model_Name_Check"].ToString();
                    break;

                case "USER":
                    lTempFileName = ConfigurationManager.AppSettings["Excel_Model_Name_User"].ToString();
                    break;
            ...
            if (!lModelFilePath.EndsWith("\\"))
            {
                lModelFilePath += "\\";
            }

            byte[] fileBytes = System.IO.File.ReadAllBytes(lModelFilePath + lTempFileName);
            string fileName = lTempFileName;
            return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, fileName);
        }
            ...
            try
            {
                if (Request.Files.Count > 0)
                {
                    ...
                    foreach (string lKey in lFileDic.Keys)
                    {
                        ...
                        //插入计划文件主表T6_Plan
                        if (AddPlan(Request.Form["txtYM"].ToString(), lFileDic[lKey], lKey, ref lOldPlanID, ref lNewPlanID, ref lSQLDic))
                        {
        ...
        //2、插入新计划，并带出新计划主表ID
        private Boolean AddPlan(String pYM, String pFileName, String pFileNameS, ref string pOldPlanID, ref string pNewPlanID, ref Dictionary<String, object> pSQLDic)
        {
            ...
            lPlan.ID = "";
            lPlan.YM = pYM;
            lPlan.FileName = pFileName;
            lPlan.FileNameS = pFileNameS;
            lPlan.UploadTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

            if (!lPlan.GetOldPlanID())
            {
                return false;
            }
            pOldPlanID = lPlan.ID;

            if (!lPlan.GetNewPlanID())
            {
                return false;
            }
            pNewPlanID = lPlan.ID;
            ...
            {
                lPlan.ID = pOldPlanID;
                pSQLDic.Add(lPlan.GetDeleteSQL(), null);
            }

            lPlan.ID = pNewPlanID;
            pSQLDic.Add(lPlan.GetInsertSQL(), null);
        ...
        #region 导入【计划-b1_总表】
        private Boolean Import_ZongBiao(DataTable pDT, String pOldPlanID, String pNewPlanID, ref Dictionary<String, object> pSQLDic)
        {
            ...
            {
                //删除老计划-总表数据
                lZongBiao.PID = pOldPlanID;
                pSQLDic.Add(lZongBiao.DeleteByPID(), null);
            }

            for (int lRowIndex = 1; lRowIndex < pDT.Rows.Count; lRowIndex++)
            {
                lZongBiao.PID = pNewPlanID;
                ...
                lZongBiao.BYJH = pDT.Rows[lRowIndex][4].ToString();
                if (!IsDigitalStr(lZongBiao.BYJH))
                {
                    if (lZongBiao.BYJH.Trim() == "")
                    {
                    ...
                    lZongBiao.BYJH = pDT.Rows[lRowIndex][5].ToString();
                    ...
                    lZongBiao.BYJH = pDT.Rows[lRowIndex][7].ToString();
        ...
        #region 导入【计划-b3_采掘作业】
        private Boolean Import_CaiJue(DataTable pDT, String pOldPlanID, String pNewPlanID, ref Dictionary<String, object> pSQLDic)
        {
            DataTable_ClearEmptyRow(ref pDT);
            DataTable_ClearEmptyColumn(ref pDT);
            ...
            {
                //删除老计划-采掘作业数据
                lCaiJue.PID = pOldPlanID;
                pSQLDic.Add(lCaiJue.DeleteByPID(), null);
            }
            ...
            for (int lRow = 2; lRow < pDT.Rows.Count; lRow++)
            {
                ...
                lCaiJue.NDJH = pDT.Rows[lRow][3].ToString();
        ...
        #region 导入【计划-b4_掘进】
        private Boolean Import_JueJin(DataTable pDT, String pOldPlanID, String pNewPlanID, ref Dictionary<String, object> pSQLDic)
        {
            DataTable_ClearEmptyRow(ref pDT);
            DataTable_ClearEmptyColumn(ref pDT);
            ...
            {
                //删除老计划-掘进数据
                lJueJin.PID = pOldPlanID;
                pSQLDic.Add(lJueJin.DeleteByPID(), null);
            }
        ...
        #region 导入【计划-b6_采矿】
        private Boolean Import_CaiKuang(DataTable pDT, String pOldPlanID, String pNewPlanID, ref Dictionary<String, object> pSQLDic)
        {
            DataTable_ClearEmptyRow(ref pDT);
            DataTable_ClearEmptyColumn(ref pDT);
            ...
            {
                //删除老计划-采掘数据
                lCaiKuang.PID = pOldPlanID;
                pSQLDic.Add(lCaiKuang.DeleteByPID(), null);
            }
        ...
        #region 导入【计划-b7_出矿】
        private Boolean Import_ChuKuang(DataTable pDT, String pOldPlanID, String pNewPlanID, ref Dictionary<String, object> pSQLDic)
        {
            DataTable_ClearEmptyRow(ref pDT);
            DataTable_ClearEmptyColumn(ref pDT);
            ...
            {
                //删除老计划-出矿数据
                lChuKuang.PID = pOldPlanID;
                pSQLDic.Add(lChuKuang.DeleteByPID(), null);
            }
